...
include("../conectar.php");
  ...
  $queryplantelH = "SELECT * FROM directorio where id_directorio = '1' LIMIT 1  
...
$este_Es_el_dir = $row_datos_queryplantelH['directorio'];

$este_periodooo = $row_datos_queryplantelH['dir_periodo'];
$este_messs = $row_datos_queryplantelH['dir_mes'];
$este_atributooo = $row_datos_queryplantelH['dir_atributo'];


mysqli_close($enlace);


// colocar aqui el nombre colocado al mes y luego colocarlo junto con el indicativo del mes
                ...
                <div class="row justify-content-md-center h-100">
                    <div class="card-wrapper">
                            ...
                            <div class="card-body">
                                <form class="form-signin" action="" method="post" autocomplete="off">
                                    <div class="form-group">
                                       <div class="brand">
                                            <svg version="1.0" xmlns="http://www.w3.org/2000/svg" M1008 width="100%" height="80px" viewBox="0 0 238.000000 140.000000" aria-label="">
                        ...
                        <div class="footer text-center">
                            &mdash;&mdash; &copy;
                            <a href=""  class="text-muted" data-version="<?php echo VERSION; ?>"></a> &mdash;&mdash;
                        </div>
                    </div>
                </div>
            </div>
...
if (isset($_POST['ajax']) && !FM_READONLY) {
    ...
    if(isset($_POST['type']) && $_POST['type'] == "upload" && !empty($_REQUEST["uploadurl"])) {
        ...
        if(!$isFileAllowed) {
            $err = array("message" => "Extension de archivo no permitida");
            event_callback(array("fail" => $err));
            exit();
        }
...
if (isset($_GET['del']) && !FM_READONLY) {
    ...
    if ($del != '' && $del != '..' && $del != '.') {
        ...
        fm_set_msg(lng('Nombre de archivo o carpeta invalido'), 'error');
    }
    fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
}
        ...
        if ($_GET['type'] == "file") {
            if (!file_exists($path . '/' . $new)) {
                if(fm_is_valid_ext($new)) {
                    ...
                    fm_set_msg(sprintf(lng('Archivo').' <b>%s</b> '.lng('Created'), fm_enc($new)));
                } else {
                    fm_set_msg(lng('Extension de archivo no permitida'), 'error');
                }
            } else {
                fm_set_msg(sprintf(lng('Archivo').' <b>%s</b> '.lng('ya existe'), fm_enc($new)), 'alert');
            }
        } else {
            if (fm_mkdir($path . '/' . $new, false) === true) {
                fm_set_msg(sprintf(lng('Carpeta').' <b>%s</b> '.lng('Creada'), $new));
            } elseif (fm_mkdir($path . '/' . $new, false) === $path . '/' . $new) {
                fm_set_msg(sprintf(lng('Carpeta').' <b>%s</b> '.lng('ya existe'), fm_enc($new)), 'alert');
            } else {
                fm_set_msg(sprintf(lng('Carpeta').' <b>%s</b> '.lng('no creada'), fm_enc($new)), 'error');
            }
        }
    } else {
        fm_set_msg(lng('Caracater invalido en nombre de archivo o carpeta'), 'error');
    }
    fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
}
...
if (isset($_GET['copy'], $_GET['finish']) && !FM_READONLY) {
    ...
    if ($from != $dest) {
        ...
        if ($move) { // Move and to != from so just perform move
            ...
            if ($rename) {
                fm_set_msg(sprintf(lng('Movido desde').' <b>%s</b> '.lng('to').' <b>%s</b>', fm_enc($copy), fm_enc($msg_from)));
            } elseif ($rename === null) {
                fm_set_msg(lng('Archivo o carpeta en esta dirección ya existe'), 'alert');
            } else {
                fm_set_msg(sprintf(lng('Error al mover desde').' <b>%s</b> '.lng('a').' <b>%s</b>', fm_enc($copy), fm_enc($msg_from)), 'error');
            }
        } else { // Not move and to != from so copy with original name
            if (fm_rcopy($from, $dest)) {
                fm_set_msg(sprintf(lng('Copiado desde').' <b>%s</b> '.lng('a').' <b>%s</b>', fm_enc($copy), fm_enc($msg_from)));
            } else {
                fm_set_msg(sprintf(lng('Error al copiar desde').' <b>%s</b> '.lng('a').' <b>%s</b>', fm_enc($copy), fm_enc($msg_from)), 'error');
            }
        }
    } else {
...
if (isset($_GET['ren'], $_GET['to']) && !FM_READONLY) {
    ...
    if (fm_isvalid_filename($new) && $old != '' && $new != '') {
        if (fm_rename($path . '/' . $old, $path . '/' . $new)) {
            fm_set_msg(sprintf(lng('Renombrado desde').' <b>%s</b> '. lng('to').' <b>%s</b>', fm_enc($old), fm_enc($new)));
        } else {
            fm_set_msg(sprintf(lng('Error al ranombrar desde').' <b>%s</b> '. lng('to').' <b>%s</b>', fm_enc($old), fm_enc($new)), 'error');
        }
    } else {
        fm_set_msg(lng('Caracter invalido en el nombre'), 'error');
    }
    fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
}
...
<link href="01 css/dropzone.min.css" rel="stylesheet"  >
        ...
        <div class="card mb-2 fm-upload-wrapper <?php echo fm_get_theme(); ?>">
            <div class="card-header">
                <ul class="nav nav-tabs card-header-tabs">
                    <li class="nav-item">
                        <a class="nav-link active" href="#fileUploader" data-target="#fileUploader"><i class="fa fa-arrow-circle-o-up"></i> <?php echo lng('Cargando') ?></a>
                    </li>
            ...
            <div class="card-body">
                <p class="card-text">
                    <a href="?p=<?php echo FM_PATH ?>" class="float-right"><i class="fa fa-chevron-circle-left go-back"></i> <?php echo lng('Regresar')?></a>
                    <?php echo lng('Carpeta de destino') ?>: <?php echo fm_enc(fm_convert_win(FM_PATH)) ?>
                </p>
    ...
    <script src="01 css/alt/dropzone.min.js"></script>
        ...
        <div class="card <?php echo fm_get_theme(); ?>">
            ...
            <div class="card-body">
                <form action="" method="post">
                    ...
                    <p>
                        <button type="submit" class="btn btn-success"><i class="fa fa-check-circle"></i> <?php echo lng('Copiar') ?></button> &nbsp;
    ...
    <div class="col-md-8 offset-md-2 pt-3">
        <div class="card mb-2 <?php echo fm_get_theme(); ?>">
            <h6 class="card-header">
                ...
                <a href="?p=<?php echo FM_PATH ?>" class="float-right"><i class="fa fa-window-close"></i> <?php echo lng('Cancelar')?></a>
            </h6>
            <div class="card-body">
            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-xs-12 col-sm-6">
                        <p><h3><a href="" class="app-v-title"></a></h3></p>
                        <p></p>
                        <p></p>
                    </div>
                    <div class="col-xs-12 col-sm-6">
                        <div class="card">
                            <ul class="list-group list-group-flush">
                                <li class="list-group-item"><a href="" ><i class="fa fa-question-circle"></i> </a> </li>
                                <li class="list-group-item"><a href="" ><i class="fa fa-bug"></i> </a></li>
                                <li class="list-group-item"><a href="javascript:latest_release_info('<?php echo VERSION; ?>');"><i class="fa fa-link"> </i> </a></li>
                                <?php if(!FM_READONLY) { ?>
        ...
        <div class="col-12">
            <?php if(!$quickView) { ?>
                ...
                <p>
                    ...
                    <b><a href="<?php echo fm_enc($file_url) ?>" target="_blank"><i class="fa fa-external-link-square"></i> <?php echo lng('Abrir') ?></a></b>
                    &nbsp;
                    <?php
                    // ZIP actions
                    ...
                    <?php } ?>
                    <b><a href="?p=<?php echo urlencode(FM_PATH) ?>"><i class="fa fa-chevron-circle-left go-back"></i> <?php echo lng('Regresar') ?></a></b>
                </p>
                <?php
            }
            if($is_onlineViewer) {
...
<form action="" method="post" class="pt-3">
            ...
            foreach ($folders as $f) {
                ...
                <tr>
                    ...
                    <td class="inline-actions"><?php if (!FM_READONLY): ?>
                            ...
                            <a title="<?php echo lng('Renombrar')?>" href="#" onclick="rename('<?php echo fm_enc(addslashes(FM_PATH)) ?>', '<?php echo fm_enc(addslashes($f)) ?>');return false;"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <?php
                         ...
                         <a href="?p=<?php echo urlencode(FM_PATH) ?>&amp;dl=<?php echo urlencode($f) ?>" data-preview-image="<?php echo $imagePreview ?>" title="<?php echo fm_enc($f) ?>">
                           <?php else: ?>
                                <a href="?p=<?php echo urlencode(FM_PATH) ?>&amp;dl=<?php echo urlencode($f) ?>" title="<?php echo $f ?>">
                            <?php endif; ?>
                    ...
                    <td class="inline-actions">
                        ...
                        <?php if (!FM_READONLY): ?>
                            <a title="<?php echo lng('Borrar') ?>" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;del=<?php echo urlencode($f) ?>" onclick="return confirm('<?php echo lng('Delete').' '.lng('File').'?'; ?>\n \n ( <?php echo urlencode($f) ?> )');"> <i class="fa fa-trash-o"></i></a>
                            <a title="<?php echo lng('Renombrar') ?>" href="#" onclick="rename('<?php echo fm_enc(addslashes(FM_PATH)) ?>', '<?php echo fm_enc(addslashes($f)) ?>');return false;"><i class="fa fa-pencil-square-o"></i></a>
                           
                        <?php endif; ?>
                      
                        <a title="<?php echo lng('Descargar') ?>"
         href="?p=<?php echo urlencode(FM_PATH) ?>&amp;dl=<?php echo urlencode($f) ?>"><i class="fa fa-download"></i></a>
    ...
    <div class="row">
        ...
        <div class="col-3 d-none d-sm-block"><a href=""  class="float-right text-muted"></a></div>
        <?php else: ?>
            <div class="col-12"><a href=""  class="float-right text-muted"></a></div>
        <?php endif; ?>
    </div>
...
function fm_show_nav_path($path)
{
    ...
    <nav class="navbar navbar-expand-lg <?php echo $getTheme; ?> mb-4 main-nav <?php echo $isStickyNavBar ?>">
        <a class="navbar-brand" href="zona_adm_pagos.php"> ¡ Regresar ! </a>
        ...
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
            ...
            <div class="col-xs-6 col-sm-7 text-right">
                <ul class="navbar-nav mr-auto float-right <?php echo fm_get_theme();  ?>">
                    ...
                    <?php if (!FM_READONLY): ?>
                    <li class="nav-item">
                        <a title="<?php echo lng('Upload') ?>" class="nav-link" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;upload"><i class="fa fa-cloud-upload" aria-hidden="true"></i> <?php echo lng('Cargar') ?></a>
                    </li>
                    ...
                    <li class="nav-item avatar dropdown">
                        ...
                        <div class="dropdown-menu dropdown-menu-right <?php echo fm_get_theme(); ?>" aria-labelledby="navbarDropdownMenuLink-5">
                            <?php if (!FM_READONLY): ?>
                            <a title="<?php echo lng('Configuración') ?>" class="dropdown-item nav-link" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;settings=1"><i class="fa fa-cog" aria-hidden="true"></i> <?php echo lng('Configuración') ?></a>
                            <?php endif ?>
...
<head>
    ...
    <meta name="description" content="Web based File Manager in PHP, Manage your files efficiently">
...
<link rel="stylesheet"  href="01 css/bootstrap.min.css">
...
<div id="wrapper" class="container-fluid">
 ...
 <script src="01 css/jquery.slim.min.js"></script>
  <script src="01 css/bootstrap.min.js"></script>
...
<link rel="stylesheet"  href="01 css/alt/bootstrap.min.css">

    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">


<link rel="stylesheet"  href="01 css/alt/ekko-lightbox.css">



    <?php if (FM_USE_HIGHLIGHTJS): ?>
  <link rel="stylesheet"  href="01 css/alt/vs.min.css">
    <?php endif; ?>
    <style>
...
<div id="wrapper" class="container-fluid">
 ...
 <script src="01 css/altb/jquery.min.js"></script>
  <script src="01 css/altb/bootstrap.min.js"></script>
   <script src="01 css/altb/jquery.dataTables.min.js"></script>
    <script src="01 css/altb/ekko-lightbox.min.js"></script>
...
<?php if (FM_USE_HIGHLIGHTJS): ?>
  ...
  <script src="01 css/altb/highlight.min.js"></script>
...
<?php if (isset($_GET['edit']) && isset($_GET['env']) && FM_EDIT_FILE):
 ...
 <script src="01 css/altb/ace.js"></script>
...
function lng($txt) {
    ...
    $tr['en']['Perms']          = 'Perms';                  $tr['en']['Modified']           = 'Modificado';
    $tr['en']['Owner']          = 'Owner';                  $tr['en']['Search']             = 'Buscar';
    $tr['en']['NewItem']        = 'Nuevo Item';               $tr['en']['Folder']             = 'Carpeta';
    $tr['en']['Delete']         = 'Borrar';                 $tr['en']['Rename']             = 'Renombrar';
    $tr['en']['CopyTo']         = 'Copiar a';                $tr['en']['DirectLink']         = 'Direct link';
    $tr['en']['UploadingFiles'] = 'Cargar Archivos';           $tr['en']['ChangePermissions']  = 'Change Permissions';
    $tr['en']['Copying']        = 'Copiando';                $tr['en']['CreateNewItem']      = 'Create New Item';
    $tr['en']['Name']           = 'Nombre';                   $tr['en']['AdvancedEditor']     = 'Advanced Editor';
    $tr['en']['RememberMe']     = 'Remember Me';            $tr['en']['Actions']            = 'Acciones';
    ...
    $tr['en']['CreateNow']      = 'Create Now';             $tr['en']['Download']           = 'Descargar';
    $tr['en']['Open']           = 'Abrir';                   $tr['en']['UnZip']              = 'UnZip';
    $tr['en']['UnZipToFolder']  = 'UnZip a';        $tr['en']['Edit']               = 'Editar';
    $tr['en']['NormalEditor']   = 'Normal Editor';          $tr['en']['BackUp']             = 'Back Up';
    $tr['en']['SourceFolder']   = 'Source Folder';          $tr['en']['Files']              = 'Archivos';
    $tr['en']['Move']           = 'Mover';                   $tr['en']['Change']             = 'Cambiar';
    $tr['en']['Settings']       = 'Settings';               $tr['en']['Language']           = 'Language';
    $tr['en']['Folder is empty']    = 'Carpeta Vacia';    $tr['en']['PartitionSize']      = 'Tamaño de disco';
    $tr['en']['ErrorReporting'] = 'Error Reporting';        $tr['en']['ShowHiddenFiles']    = 'Show Hidden Files';
    $tr['en']['Full size']      = 'Tamaño Completo';              $tr['en']['Help']               = 'Help';
    $tr['en']['Free of']        = 'Libre de';                $tr['en']['Preview']            = 'Preview';
    $tr['en']['Help Documents'] = 'Help Documents';         $tr['en']['Report Issue']       = 'Report Issue';
    $tr['en']['Generate']       = 'Generar';               $tr['en']['FullSize']           = 'Tamaño Completo';
    $tr['en']['FreeOf']         = 'libre de';                $tr['en']['CalculateFolderSize']= 'Calculate folder size';
    $tr['en']['ProcessID']      = 'Process ID';             $tr['en']['Created']    = 'Creado';
    ...
    $tr['en']['Nothing selected']   = 'Nada seleccionado';   $tr['en']['Paths must be not equal']    = 'Paths must be not equal';
    $tr['en']['Renamed from']       = 'Renombrado desde';       $tr['en']['Archive not unpacked']       = 'Archive not unpacked';
    $tr['en']['Deleted']            = 'Borrado';            $tr['en']['Archive not created']        = 'Archivo no creado';
    $tr['en']['Copied from']        = 'Copiado desde';        $tr['en']['Permissions changed']        = 'Permissions changed';
    $tr['en']['to']                 = 'a';                 $tr['en']['Saved Successfully']         = 'Guardado con exito';
    $tr['en']['not found!']         = 'no encontrado!';         $tr['en']['File Saved Successfully']    = 'Archivo Guardado';
    $tr['en']['Archive']            = 'Archivado';            $tr['en']['Permissions not changed']    = 'Permissions not changed';
    $tr['en']['Select folder']      = 'Seleccionar carpeta';      $tr['en']['Source path not defined']    = 'Source path not defined';
    $tr['en']['already exists']     = 'ya existe';     $tr['en']['Error while moving from']    = 'Error while moving from';
    $tr['en']['Create archive?']    = 'Crear archivo?';    $tr['en']['Invalid file or folder name']    = 'Archivo o carpeta invalida';
    $tr['en']['Archive unpacked']   = 'Archive unpacked';   $tr['en']['File extension is not allowed']  = 'Tipo de archivo no permitido';
    $tr['en']['Root path']          = 'Root path';          $tr['en']['Error while renaming from']  = 'Error while renaming from';
    $tr['en']['File not found']     = 'Archivo no encontrado';     $tr['en']['Error while deleting items'] = 'Error al borrar';
    $tr['en']['Invalid characters in file name']                = 'Caracter invalido en el nombre del archivo';
    $tr['en']['FILE EXTENSION HAS NOT SUPPORTED']               = 'LA EXTENSION DEL ARCHIVO NO ESTA SOPORTADA';
    $tr['en']['Selected files and folder deleted']              = 'Archivo y carpeta borrado';
    $tr['en']['Error while fetching archive info']              = 'Error while fetching archive info';
    $tr['en']['Delete selected files and folders?']             = 'Borrar los archivos y carpetas seleccionados?';
    $tr['en']['Search file in folder and subfolders...']        = 'Search file in folder and subfolders...';
    $tr['en']['Access denied. IP restriction applicable']       = 'Access denied. IP restriction applicable';
    $tr['en']['Invalid characters in file or folder name']      = 'Caracter invalido en nombre de archivo o carpeta';
    $tr['en']['Operations with archives are not available']     = 'Operations with archives are not available';
    $tr['en']['File or folder with this path already exists']   = 'Archivo o carpeta con este nombre ya existe';

    $tr['en']['Moved from']                 = 'Movido desde';
...
?>
    ...
    Explorando: <b><?php echo $este_periodooo; ?></b>, Mes: "<?php echo $este_messs; ?>" - Corte: <?php echo $este_atributooo;  ?>.
    </div>

<p class="text-danger ml-3"><b>Info:</b> El sistema solo permite cargar y manipular archivos en formato PDF.</p>