...
<?php include ("listas_imp_php/".$consul." "); ?>  <!-- hace la lista -->
  ...
  <div class="container-fluid">
    ...
    <div class="form-row">
        ...
        <div class="form-row ml-1">
              ...
              <div data-toggle="tooltip" data-placement="top" title="Lista con información" >

    <form target="_blank" action="zzz_impresiones/imp_lista_personal02.php" method="POST">

    <input type="hidden" id="consul"   name="consul"   value="<?php echo $consul;?>">
    <input type="hidden" id="titulo"   name="titulo"   value="<?php echo $ttitulo;?>">
<!--    <input type="hidden" id=""   name=""   value="">
    <input type="hidden" id=""   name=""   value="">  -->


          <button type="submit" name="imp_lista_del_personal" id="imp_lista_del_personal" class="btn btn-primary mr-1 mb-1 btn_sizes"  >
           <i class="fas fa-print"></i>&nbsp;&nbsp; "Completo"</button>
  ...
  <div data-toggle="tooltip" data-placement="top" title="Lista con nombres" >

    <form target="_blank" action="zzz_impresiones/imp_lista_personal02_vac.php" method="POST">
...
<!--    <input type="hidden" id=""   name=""   value="">
    <input type="hidden" id=""   name=""   value="">  -->
          ...
          <button type="submit" name="imp_lista_del_personal_vac" id="imp_lista_del_personal_vac" class="btn btn-success mr-1 mb-1 btn_sizes"  >
           <i class="fas fa-print"></i>&nbsp;&nbsp; "Básico"</button>
...
mysqli_close($enlace); 
                  ...
                  do  {   ?>    <!-- va generarme tantas td como cargos tenga en la DB maximo son 2 -->


                  <td class="align-middle" align="center"><?php echo $row_datos_plantillaH['nombre_cargo']; ?> </td>
...
$queryHT = "SELECT * FROM titulos_all, plantilla_titulos, plantilla_mencion
                        
          WHERE  plantilla_titulos.id_titulos = titulos_all.id_titulos
          and    plantilla_mencion.id_mencion = titulos_all.id_mencion
          and    titulos_all.id_per = '$id_perx'
          

            ";

$datos_plantillaHT = mysqli_query($enlace, $queryHT) or die(mysqli_error());

$row_datos_plantillaHT = mysqli_fetch_array($datos_plantillaHT); 
...
mysqli_close($enlace); 
                  ...
                  <td class="align-middle" align="center"><?php echo $row_datos_plantillaHT['nombre_titulos'];
                                                                echo " ". $row_datos_plantillaHT['nombre_mencion'];
                                                               ?> </td>
                  
                  

                  <?php }    while ($row_datos_plantillaHT = mysqli_fetch_assoc($datos_plantillaHT));  ?>   
...
<div class="form-row mb-2">
  ...
  <button type="button" class="btn btn-outline-info btn-sm" data-toggle="modal"
          data-target="#modificar<?php echo $row_datos_plantilla['id_per']; ?>">
...
<script>  // ayuda a que al cerrar este modal se refresque la pagina y asi funciona bien la detencion de si es profesor o no
$(document).ready(function(){
    $("#modificar<?php echo $row_datos_plantilla['id_per']; ?>").click(function(){
        $("#modificar<?php echo $row_datos_plantilla['id_per']; ?>").modal("show");
    });
        $("#modificar<?php echo $row_datos_plantilla['id_per']; ?>").on('hide.bs.modal', function () {
...
/*
$(document).ready(function(){
    $("#foto_update<?php echo $row_datos_plantilla['id_per']; ?>").click(function(){
        $("#foto_update<?php echo $row_datos_plantilla['id_per']; ?>").modal("show");
    });
        $("#foto_update<?php echo $row_datos_plantilla['id_per']; ?>").on('hide.bs.modal', function () {
           window.location = window.location.href;       //  antes  = "plantilla_personal_global.php"
    });
}); */
...
<!-- ayuda a conseguir los datos de las direcciones -->


<?php

$id_de_direccion = $row_datos_plantilla['id_direccion'];
$id_del_paisFGH = $row_datos_plantilla['d_paises'];

include("../conectar.php");   

$queryDDD = "SELECT * FROM direcciones, d_ciudades, d_municipio, d_parroquia, d_avenidas,
                        d_codigo, d_vivienda, d_piso, d_numero
                        
         WHERE  direcciones.id_direccion = '$id_de_direccion'
...
$datos_plantillaDDD = mysqli_query($enlace, $queryDDD) or die(mysqli_error());

$row_datos_plantillaDDD = mysqli_fetch_array($datos_plantillaDDD); 





$queryPPP = "SELECT * FROM d_paises
                        
         WHERE  id_pais = '$id_del_paisFGH'
            
            ";

$datos_plantillaPPP = mysqli_query($enlace, $queryPPP) or die(mysqli_error());

$row_datos_plantillaPPP = mysqli_fetch_array($datos_plantillaPPP); 
...
<div class="modal fade" id="modificar<?php echo $row_datos_plantilla['id_per']; ?>" tabindex="-1" role="dialog" aria-hidden="true"  >
  <div class="modal-dialog modal-xl" role="document">
    <div class="modal-content">

      <div class="modal-header">
        <h5 class="modal-title text-info" id="exampleModalLabel">
        <i class="fas fa-info fa-lg"></i> &nbsp;Editar o Modificar!!! <?php echo $row_datos_plantilla['id_contrato']; ?></h5>
      ...
      <div class="modal-body">
       ...
       <?php include("plantilla_personal_00_mod.php"); ?>

        <input type="hidden" id="idzzz"  name="idzzz" value="<?php echo $row_datos_plantilla['id_per']; ?>">
        <input type="hidden" id="contratozzz"  name="contratozzz" value="<?php echo $row_datos_plantilla['id_contrato']; ?>">

      </div>  <!-- cierre modal body -->
      <div class="modal-footer">

        <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancelar</button>
        <button type="submit" name="editar_selecc" class="btn btn-info" value="<?php echo $row_datos_plantilla['id_per']; ?>">
              Modificar</button>  
...
$queryHU = "SELECT * FROM cargos_all, plantilla_cargos
...
$datos_plantillaHU = mysqli_query($enlace, $queryHU) or die(mysqli_error());

$row_datos_plantillaHU = mysqli_fetch_array($datos_plantillaHU); 
...
<?php if ($row_datos_plantillaHU['id_grupo']=='5' OR $row_datos_plantillaHU['id_grupo']=='1' OR $row_datos_plantillaHU['id_grupo']=='9' OR $row_datos_plantillaHU['id_grupo']=='14') { ?>
                       <!-- si esta en el grupo de profesores, o directores  o dpto evaluacion -->



    <form target="_blank" action="zzz_impresiones/imp_plantilla.php" method="POST">





<input type="hidden" id="id_perxz" name="id_perxz" value="<?php echo $row_datos_plantilla['id_per']; ?>">
<input type="hidden" id="id_contraxz" name="id_contraxz" value="<?php echo $row_datos_plantilla['id_contrato']; ?>">


 <?php 
             if ($row_datos_plantilla["foto_per"] == "")             //  si no hay foto carga la foto vacia
                          {
                             $fotoZ = "../zz_fotos_p/04vacio.png ";                             
                          }
                else { $fotoZ = $row_datos_plantilla["foto_per"]; }
?>



<input type="hidden" id="foto_perxz" name="foto_perxz" value="<?php echo $fotoZ; ?>">

<input type="hidden" id="nombre_perxz" name="nombre_perxz" value="<?php echo $row_datos_plantilla['nombre_per']; ?>">
<input type="hidden" id="apellido_perxz" name="apellido_perxz" value="<?php echo $row_datos_plantilla['apellido_per']; ?>">
<input type="hidden" id="ci_perxz" name="ci_perxz" value="<?php echo $row_datos_plantilla['ci_per']; ?>">
<input type="hidden" id="nombre_paisxz" name="nombre_paisxz" value="<?php echo $row_datos_plantillaPPP['nombre_pais']; ?>">

<input type="hidden" id="nombre_avxz" name="nombre_avxz" value="<?php echo $row_datos_plantillaDDD['nombre_av']; ?>">
<input type="hidden" id="dir_calle_sectorxz" name="dir_calle_sectorxz" value="<?php echo $row_datos_plantillaDDD['dir_calle_sector']; ?>">
<input type="hidden" id="nombre_viviendaxz" name="nombre_viviendaxz" value="<?php echo $row_datos_plantillaDDD['nombre_vivienda']; ?>">
<input type="hidden" id="dir_nombre_viviendaxz" name="dir_nombre_viviendaxz" value="<?php echo $row_datos_plantillaDDD['dir_nombre_vivienda']; ?>">
<input type="hidden" id="nombre_pisoxz" name="nombre_pisoxz" value="<?php echo $row_datos_plantillaDDD['nombre_piso']; ?>">
<input type="hidden" id="nombre_numeroxz" name="nombre_numeroxz" value="<?php echo $row_datos_plantillaDDD['nombre_numero']; ?>">
<input type="hidden" id="nombre_municipioxz" name="nombre_municipioxz" value="<?php echo $row_datos_plantillaDDD['nombre_municipio']; ?>">
<input type="hidden" id="nombre_parroquiaxz" name="nombre_parroquiaxz" value="<?php echo $row_datos_plantillaDDD['nombre_parroquia']; ?>">



<input type="hidden" id="celular_perxz" name="celular_perxz" value="<?php echo $row_datos_plantilla['celular_per']; ?>">
<input type="hidden" id="email_perxz" name="email_perxz" value="<?php echo $row_datos_plantilla['email_per']; ?>">


<input type="hidden" id="nombre_cargoxz" name="nombre_cargoxz" value="<?php echo $row_datos_plantillaF['nombre_cargo']; ?>">




<?php 
             if ($row_datos_plantillaESTU['nombre_respuesta'] == "Si")             //  si no hay foto carga la foto vacia
                          {
                             $respuestaZ = "SI<u>  X  </u> NO______";                             
                          }
                else { $respuestaZ = "SI______ NO<u>  X  </u>"; }
?>



<input type="hidden" id="nombre_respuestaxz" name="nombre_respuestaxz" value="<?php echo $respuestaZ; ?>">

<input type="hidden" id="nombre_institutoxz" name="nombre_institutoxz" value="<?php echo $row_datos_plantillaESTU['nombre_instituto']; ?>">
<input type="hidden" id="nombre_titulosxz" name="nombre_titulosxz" value="<?php echo $row_datos_plantillaESTU['nombre_titulos']; ?>">
<input type="hidden" id="nombre_semestrexz" name="nombre_semestrexz" value="<?php echo $row_datos_plantillaESTU['nombre_semestre']; ?>">



<?php
$fecha_form_vid = date('d-m-Y', strtotime($row_datos_plantillaQQ['fecha_vida']));
$fecha_form_contra = date('d-m-Y', strtotime($row_datos_plantilla['contratado_desde']));
?>


<input type="hidden" id="fecha_vidaxz" name="fecha_vidaxz" value="<?php echo $fecha_form_vid; ?>">
<input type="hidden" id="plantel_vidaxz" name="plantel_vidaxz" value="<?php echo $row_datos_plantillaQQ['plantel_vida']; ?>">
<input type="hidden" id="contratado_desdexz" name="contratado_desdexz" value="<?php echo $fecha_form_contra; ?>">




<?php 
             if ($row_datos_plantillaOTR['nombre_respuesta'] == "Si")             //  si no hay foto carga la foto vacia
                          {
                             $respuestaX = "SI<u>  X  </u> NO______";                             
                          }
                else { $respuestaX = "SI______ NO<u>  X  </u>"; }
?>


<input type="hidden" id="nombre_respuestaXxz" name="nombre_respuestaXxz" value="<?php echo $respuestaX; ?>">




<?php
$edadtrabajando = $row_datos_plantillaQQ['edad_oficiales']+$row_datos_plantillaQQ['edad_privados'];

?>


<input type="hidden" id="edad_oficialesxz" name="edad_oficialesxz" value="<?php echo $row_datos_plantillaQQ['edad_oficiales']; ?>">
<input type="hidden" id="edad_privadosxz" name="edad_privadosxz" value="<?php echo $row_datos_plantillaQQ['edad_privados']; ?>">

<input type="hidden" id="edadtrabajandoxz" name="edadtrabajandoxz" value="<?php echo $row_datos_plantillaQQ['edad_total']; ?>">

<input type="hidden" id="observacionesxz" name="observacionesxz" value="<?php echo $row_datos_plantillaOBS['obs_per']; ?>">

<input type="hidden" id="tiempo_de_trabajoxz" name="tiempo_de_trabajoxz" value="<?php echo $row_datos_plantillaOBS['total_horas']; ?>">
      ...
      <button type="submit" name="impx"
      class="btn btn-outline-warning btn-sm"  id="impx">
...
<?php if ($row_datos_plantillaHU['id_grupo']!='5' AND $row_datos_plantillaHU['id_grupo']!='1' AND $row_datos_plantillaHU['id_grupo']!='9' AND $row_datos_plantillaHU['id_grupo']!='14') { ?>
                       <!-- cualquier otro grupo -->


                    
                    <form target="_blank" action="zzz_impresiones/imp_plantilla_usual.php" method="POST">
...
<input type="hidden" id="edad_oficialesxz" name="edad_oficialesxz" value="<?php echo $row_datos_plantillaQQ['edad_oficiales']; ?>">
<input type="hidden" id="edad_privadosxz" name="edad_privadosxz" value="<?php echo $row_datos_plantillaQQ['edad_privados']; ?>">
<input type="hidden" id="edadtrabajandoxz" name="edadtrabajandoxz" value="<?php echo $edadtrabajando; ?>">
...
<input type="hidden" id="tiempo_de_trabajoxz" name="tiempo_de_trabajoxz" value="<?php echo $row_datos_plantillaOBS['total_horas']; ?>">
                    ...
                    <button type="submit" name="impxu"
                    class="btn btn-outline-warning btn-sm"  id="impxu">
                    <i class="fas fa-print fa-lg"></i></button>
