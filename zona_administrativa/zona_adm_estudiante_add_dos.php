...
// estudiante 1
	...
	$var_cedula_re = $_POST["cedula_del_repre_es"];
...
$mod_email_estu = mysqli_real_escape_string($enlace,$_POST['mod_email_estu']);
...
$query_ES_AA = "INSERT INTO reg_estudiantes(estu_expediente, nombre_estu, apellidos_estu, nacimiento_estu, id_sexo, id_ciudad,
			                                id_estado, id_pais, id_religion, ci_estu, id_medico, id_sangre, email_estu, inscrito_si_o_no, municipio_est, vive_con, se_traslada_en, enf_padecidas, alergico_a, se_retira_con, control_med, v1, v2, v3, v4, v5, v6, v7, v8, v9, v10, v11, v12, v13, v14, v15, v_otra, medicamentos)  
							...
							VALUES (   '".mysqli_real_escape_string($enlace,$_POST['expediente_est1'])."'          ,
									   ...
									   '".mysqli_real_escape_string($enlace,$_POST['cedula_est1'])."'          ,									   
										...
										'$padece_act1', '$sangre_act1', '$mod_email_estu',  '1', '".mysqli_real_escape_string($enlace,$_POST['municipio_est1'])."' ,


									   '".mysqli_real_escape_string($enlace,$_POST['mod_vive_con_estud'])."'         ,
							           '".mysqli_real_escape_string($enlace,$_POST['mod_traslada_estud'])."'         ,
									   '".mysqli_real_escape_string($enlace,$_POST['mod_padecio_estud'])."'          ,
									   '".mysqli_real_escape_string($enlace,$_POST['mod_alergias_estud'])."'    ,

									   '$mod_se_retira_con', '$mod_control_med',
									   '$v1', '$v2', '$v3', '$v4', '$v5', '$v6', '$v7', '$v8', '$v9',
									   '$v10', '$v11', '$v12', '$v13', '$v14', '$v15',

									   '$v_otra', '$mod_medicamentos'
 ...
 $query_pagAA = "INSERT INTO reg_pagos(id_del_estud, nota_principal) VALUES ('$estu1_id', NULL)";
				           ...
				           if (!mysqli_query($enlace,$query_pagAA))  
								{
$errorZ.="- Error en crear historial. ";
...
$sqlAAAA = "DELETE FROM reg_estudiantes WHERE id_estud = '$estu1_id' "; 
							...
							$query_AA_repre = "INSERT INTO reg_estu_repre_all(id_repre, id_estu) VALUES ( '$repre_id', '$estu1_id')";
								...
								// secuencia que borra al estudiante.


								$sqlAAAAAA = "DELETE FROM reg_estudiantes WHERE id_estud = '$estu1_id' "; 

								if (mysqli_query($enlace,$sqlAAAAAA))  // si no logro asociar el estudiante 1 con su representante
								{ 									   
								$errorZ.="- &nbsp; Reg Estu Clear!!! &nbsp ";
								$conteo_errorA = "1";
								}
			 ...
			 $datosES1_repre_success="<i class=\"fas fa-user-friends fa-lg\"></i>";  // coloca success al lado de estu repre 1    
					            	...
					            	clearstatcache();
									$filename_estu1 = '../0 Croppie/foto_estu_update_'.$el_propio_id.'_'.$fotPP.'.png';
									$foto_estu1_info ="0";
															...
															if (file_exists($filename_estu1 ))     // de haber una foto le cambia el nombre y la mueve a otro lugar
																		{
																			...
																			$image_estu1 = imagecreatefrompng("../0 Croppie/foto_estu_update_".$el_propio_id.'_'.$fotPP.".png");  
																				           unlink("../0 Croppie/foto_estu_update_".$el_propio_id.'_'.$fotPP.".png");
																				           imagejpeg($image_estu1,"../0 Croppie/foto_estu_update_".$el_propio_id.'_'.$fotPP.".png",90); // comprimer la imagen  
...
$query_ES_AA_sob = "INSERT INTO reg_estu_sobre(id_estud, proviene_de_otro, nombre_plantel_pro, id_ciudad, id_grado, id_periodo_es, es_repitiente, areas_pendientes) 
							...
							VALUES (   '$estu1_id',
									   ...
									   '$grado_curso_otro1',
									   '$cursaba_est1',                                                         
									   '$old_repite_otro1', '$mod_areas_pendientes')";
				...
				if (!mysqli_query($enlace,$query_ES_AA_sob))  // si no logro ingresar sobre el estudiante 1
				            ...
				            {   // en caso de que logre realizar el registro sobre el estudiante 1
								...
								$fecha_ins_act1 = $_POST["fecha_ins_act1"];
								$seccion_act1 = $_POST["seccion_act1"];
...
$query_ES_AA_act = "INSERT INTO reg_estu_actual(id_estud, id_reg_pagos, cursa_actualmente, grado_actual, fecha_inscripcion, repite_actual, 
				...
				if (!mysqli_query($enlace,$query_ES_AA_obs))  // si no logro ingresar las observaciones del estudiante 1
				            ...
				            {   // en caso de que logre guardar las obs del estudiante 1
								...
								$obs_success1="<i class=\"fas fa-check-circle fa-lg\"></i>";
											...
											$exitoZ.="- &nbsp; Representado, inscrito sin foto. &nbsp;&nbsp; <i class=\"fas fa-user-check fa-lg\"></i> &nbsp; &nbsp; &nbsp; &nbsp;
											<b><a href=\"plantilla_representantes_search.php?tituloJKL=Resultados...&criterio=".$var_cedula_re."\"><i class=\"fas fa-link\"></i> Completar Planilla Extendida!!!</a></b>
											 ";
											...
											mysqli_close($enlace);
										    	...
										    	$exitoZ.="- &nbsp; Representado, inscrito con foto. &nbsp;&nbsp; <i class=\"fas fa-user-check fa-lg\"></i> &nbsp; &nbsp; &nbsp; &nbsp;
										    	<b><a href=\"plantilla_representantes_search.php?tituloJKL=Resultados...&criterio=".$var_cedula_re."\"><i class=\"fas fa-link\"></i> Completar Planilla Extendida!!!</a></b> ";
